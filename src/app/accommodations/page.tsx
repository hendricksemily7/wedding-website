    ...
    <div className="w-full max-w-[1100px] mx-auto rounded-lg">
      ...
      <div className="flex flex-col gap-8 items-center">
        {hotels.map((hotel) => (
          <div
            key={hotel.id}
            className="bg-[rgba(95,133,117,0.45)] flex flex-col md:flex-row items-center gap-4 group rounded-lg transition-colors duration-200 border border-gray-300 p-4 w-full shadow-sm md:h-[250px] overflow-hidden"
          >
            {/* Hotel image: link on desktop, not on mobile */}
            <a href={hotel.link} className="hidden md:block">
              <Image
                alt={hotel.name}
                src={hotel.imageSrc}
                width={180}
            ...
            {/* Show image only (not a link) on mobile */}
            ...
            <div className="w-full md:w-1/2 flex flex-col items-start text-left gap-1">
              ...
              <a
                href={hotel.link}
                className="mt-2 text-[#2D4D3A] underline font-medium text-xs md:text-sm block md:hidden"
                target="_blank"
